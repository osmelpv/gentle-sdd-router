...
export async function unifiedSync(options = {}) {
  ...
  // Step 8: pre-compile tui.tsx → tui.js (avoids Bun segfault on TSX transpilation)
  const tuiBuildStep = await buildTuiPluginStep({ dryRun, projectDir, opencodeConfigDir });
  steps.push(tuiBuildStep);
