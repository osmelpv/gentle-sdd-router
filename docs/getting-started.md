...
> **Built-in example: sdd-debug** — GSR ships with a working cross-catalog invocation. When the default SDD-Orchestrator's verify phase finds issues, it automatically invokes the `sdd-debug` catalog (5 phases: analyze-area → implant-logs → collect-and-diagnose → apply-fixes → finalize). The orchestrator retains strategic phases (collect-and-diagnose, finalize) and delegates mechanical work (analyze-area, implant-logs, apply-fixes). Key features include AI-first execution, an interactive checkpoint before code changes, and a guard position registry for deterministic cleanup. Every built-in preset has a `debug_invoke` block pre-configured — no manual wiring needed. You can do the same for your own SDDs using `gsr phase invoke` or by editing `sdd.yaml` directly. See the README for the full sdd-debug architecture.
